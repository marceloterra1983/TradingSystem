...
```yaml
# ❌ ATUAL: docker-compose.4-4-rag-stack.yml
services:
  qdrant:
...
services:
  qdrant-primary:
    image: qdrant/qdrant:latest
    container_name: rag-qdrant-primary
    ports: ["6333:6333"]
    volumes:
  ...
  qdrant-replica-1:
    image: qdrant/qdrant:latest
    container_name: rag-qdrant-replica-1
    ports: ["6334:6333"]
    volumes:
    ...
    environment:
      ...
      - QDRANT__CLUSTER__P2P__PEERS=rag-qdrant-primary:6335

  qdrant-replica-2:
    image: qdrant/qdrant:latest
    container_name: rag-qdrant-replica-2
    ports: ["6335:6333"]
    volumes:
...
for collection in documentation__nomic documentation__mxbai documentation__gemma; do
  ...
  docker exec rag-qdrant curl -X POST "http://localhost:6333/collections/$collection/snapshots" \
    -H "Content-Type: application/json"
  
  # Download snapshot
  snapshot_name="${collection}_${TIMESTAMP}.snapshot"
  docker exec rag-qdrant cat "/qdrant/storage/snapshots/$collection/latest.snapshot" \
