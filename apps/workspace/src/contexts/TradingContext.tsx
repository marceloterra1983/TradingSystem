...
export function TradingProvider({ 
  ...
  const [portfolio, setPortfolioState] = useState<Portfolio | null>(initialPortfolio);
...
export function usePortfolioActions() {
  ...
  const updatePosition = useCallback((symbol: string, newData: TradingData) => {
    if (!portfolio) return;
  ...
  const addPosition = useCallback((newPosition: TradingData) => {
    if (!portfolio) return;
  ...
  const removePosition = useCallback((symbol: string) => {
    ...
    const positionToRemove = portfolio.positions.find(pos => pos.symbol === symbol);
