#!/usr/bin/env bash
# Shell wrapper para executar o comando oficial do Kestra via docker run.
...
# Carrega variáveis do .env raiz caso existam
if [[ -f "${ENV_FILE}" ]]; then
  set -o allexport
...
mkdir -p "${STORAGE_DIR}" "${WORKDIR_DIR}/tmp"

if ! docker network inspect "${NETWORK}" >/dev/null 2>&1; then
  echo "ℹ️  Criando rede Docker ${NETWORK}..."
...
existing_id="$(docker ps -aq --filter "name=^${CONTAINER_NAME}$" | head -n1 || true)"
if [[ -n "${existing_id}" ]]; then
  if docker ps -q --filter "id=${existing_id}" >/dev/null 2>&1 && [[ -n "$(docker ps -q --filter "id=${existing_id}")" ]]; then
    echo "ℹ️  Contêiner ${CONTAINER_NAME} já está em execução (ID ${existing_id}). Reiniciando..."
  else
    echo "ℹ️  Removendo contêiner anterior ${CONTAINER_NAME} (ID ${existing_id})."
  fi
...
echo "▶️  Executando Kestra com porta ${PORT}, management ${MANAGEMENT_PORT}, rede ${NETWORK}"
