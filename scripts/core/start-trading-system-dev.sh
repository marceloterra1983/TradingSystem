...
if [ "$SKIP_FRONTEND" = false ]; then
    start_service "Dashboard Frontend" "$REPO_ROOT/frontend/dashboard" "npm run dev"
fi

if [ "$SKIP_DOCS" = false ]; then
    start_service "Docs (Docusaurus)" "$REPO_ROOT/docs" "npm run start -- --port 3400 --host 0.0.0.0"
fi
...
[ "$SKIP_DOCS" = false ] && echo "  - Docs (Docusaurus):  http://localhost:3400"
[ "$START_DOCS_DOCKER" = true ] && echo "  - Docs (Docker):      http://localhost:3400"
echo ""
