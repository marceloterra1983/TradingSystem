...
if (-not $SkipDocs) {
  Invoke-ServiceStartup -Name 'Docs (Docusaurus)' `
    ...
    -StartCommand 'npm run start -- --port 3400 --host 0.0.0.0'
}
...
if (-not $SkipDocs) { Write-Host '  - Docs (Docusaurus):  http://localhost:3400' -ForegroundColor White }
if ($StartDocsDocker) { Write-Host '  - Docs (Docker):      http://localhost:3400' -ForegroundColor White }
Write-Host ''
