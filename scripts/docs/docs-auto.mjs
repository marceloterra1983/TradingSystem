...
export async function runDocsAuto({repoRoot = resolve(__dirname, '..', '..'), logger = DEFAULT_LOGGER} = {}) {
  ...
  const designSystemDir = join(docsDir, 'content', 'frontend', 'design-system');
  const tokensPlaceholderFile = join(designSystemDir, 'tokens.mdx');
  ...
  const tasks = [
    ...
    {
      name: 'Ensure design system directory exists',
      run: async () => {
    ...
    },
    {
      ...
      run: async () => {
        const existing = await readFileIfExists(portsPlaceholderFile);
        if (existing) {
          return;
        }
        const contents = [
    ...
    {
      name: 'Seed tokens placeholder',
      run: async () => {
        const existing = await readFileIfExists(tokensPlaceholderFile);
        if (existing && existing.includes('<!-- BEGIN AUTO-GENERATED: Token Catalogue -->')) {
          return;
        }
        const contents = [
          '---',
          'title: Design Tokens',
          'description: Catálogo gerado automaticamente a partir do Tailwind do dashboard.',
          'tags:',
          '  - frontend',
          '  - design-system',
          '  - reference',
          ...
          '> Fonte: `frontend/dashboard/tailwind.config.js`',
          '',
          ...
          '| Category | Name | Value | Description |',
          '|----------|------|-------|-------------|',
          '| color | placeholder | #000000 | Placeholder token until docs:auto roda. |',
          '',
          '<!-- END AUTO-GENERATED: Token Catalogue -->',
          '',
        ].join('\n');
        await writeFile(tokensPlaceholderFile, contents, 'utf8');
      },
    },
    {
