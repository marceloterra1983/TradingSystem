    ...
    },
    {
      ...
      run: async () => {
        ...
        const stripComments = (value) =>
          removeComment(removeComment(removeComment(value, timestampLabel), sourceLabel), automationLabel).trim();
        const previousTimestamp = extractCommentValue(rest, timestampLabel);
        let updated = rest;
        ...
        const restCore = stripComments(rest);
        const updatedCore = stripComments(updated);
        const timestampToUse =
          updatedCore === restCore && previousTimestamp !== null ? previousTimestamp : timestamp;
        updated = updatedBase;
