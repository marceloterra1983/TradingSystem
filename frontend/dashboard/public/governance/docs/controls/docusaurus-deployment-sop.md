...
# ✅ CORRECT: Always rebuild docs first
...
- `/next/` → redirect to `/index.html` → not found → redirect to `/index.html` → **infinite loop**
...
```bash
# Quick health check
curl -f http://localhost:3404/health || echo "❌ Docs service unhealthy"
...
echo "📚 Docusaurus Initial Deployment"
...
echo "✅ Docusaurus deployed successfully!"
echo "🌐 Access at: http://localhost:3404"
```
...
echo "📝 Updating Docusaurus content"
...
echo "🚨 Emergency Docusaurus Recovery"

# 1. Check if build exists
if [ ! -f "docs/build/index.html" ]; then
  echo "❌ Build missing - running full rebuild"
  cd docs
  npm ci
  npm run docs:build
  cd ..
...
```
❌ User: "Docusaurus showing 500 error"
❌ User: "Documentation not loading"
❌ User: "Getting Internal Server Error on docs"
❌ Logs: "rewrite or internal redirection cycle"
❌ Health check fails: curl http://localhost:3404/health
```
...
- [ ] ✅ Verify `docs/build/index.html` exists
- [ ] ✅ Run `npm run docs:build` if content changed
- [ ] ✅ Check build timestamp is recent
- [ ] ✅ Test health endpoint after restart
- [ ] ✅ Check NGINX logs for redirect errors
...
if git diff --cached --name-only | grep -q "^docs/content/"; then
  echo "📚 Documentation files changed - validating build..."
      ...
      - name: Verify build output
        run: |
          ...
          if [ $SIZE -lt 102400 ]; then
            echo "❌ Build suspiciously small: $SIZE bytes"
            exit 1
          fi

          echo "✅ Build valid: $SIZE bytes"
...
if ! curl -sf http://localhost/ | grep -q "docusaurus"; then
  echo "❌ Homepage not serving correctly"
  exit 1
fi
