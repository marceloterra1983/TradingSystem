...
export const GatewayLogsCard: React.FC = () => {
  ...
  const getLevelIcon = (level: string) => {
    switch (level) {
      case 'INFO':
        return <CheckCircle className="h-4 w-4 text-cyan-500 dark:text-cyan-400" />;
      case 'WARN':
    ...
    <CollapsibleCard cardId="telegram-gateway-logs" defaultCollapsed={false}>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-cyan-50 rounded-lg p-3 border border-cyan-200 dark:bg-cyan-900/20 dark:border-cyan-700">
          <div className="text-sm text-cyan-600 dark:text-cyan-400 mb-1">Info</div>
          <div className="text-2xl font-bold text-cyan-700 dark:text-cyan-300">{stats.info}</div>
        </div>
        <div className="bg-amber-50 rounded-lg p-3 border border-amber-200 dark:bg-amber-900/20 dark:border-amber-700">
          <div className="text-sm text-amber-600 dark:text-amber-400 mb-1">Avisos</div>
          <div className="text-2xl font-bold text-amber-700 dark:text-amber-300">{stats.warn}</div>
        </div>
        <div className="bg-red-50 rounded-lg p-3 border border-red-200 dark:bg-red-900/20 dark:border-red-700">
          <div className="text-sm text-red-600 dark:text-red-400 mb-1">Erros</div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded ${
                    log.level === 'INFO' ? 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/40 dark:text-cyan-300' :
                    log.level === 'WARN' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300' :
                    'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300'
                  }`}>
                    {log.level}
                  </span>
                </div>
                <p className={`text-sm font-medium mb-1 ${getLevelTextColor(log.level)}`}>{log.message}</p>
                  ...
                  <div className="text-xs font-mono text-slate-600 dark:text-slate-400 bg-slate-100/50 dark:bg-slate-800/50 rounded px-2 py-1 mt-2">
                    {Object.entries(log.data).map(([key, value]) => (
                      <div key={key} className="truncate">
                        <span className="font-semibold text-slate-700 dark:text-slate-300">{key}:</span> {String(value)}
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
          ...
          <button
            onClick={() => {
              ...
              setStats(prev => ({ ...prev, lastUpdate: new Date().toISOString() }));
            }}
