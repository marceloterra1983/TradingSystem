...
export function SignalsTable() {
  ...
  const typeOptions = useMemo(() => {
    const signalTypes = new Set(
      signals.map((s) => s.signal_type).filter(Boolean),
    );
  ...
  const filteredSignals = useMemo(() => {
    ...
    filtered = filtered.filter(
      (s) => s.asset !== "__checkpoint__" && s.channel !== "system",
    );
    ...
    const fromTs = fromDate
      ? dateStringToSaoPauloTimestamp(fromDate, false)
      : undefined;
    const toTs = toDate
      ? dateStringToSaoPauloTimestamp(toDate, true)
      : undefined;
            ...
            <table className="w-full text-sm bg-white dark:bg-gray-950">
              ...
              <tbody>
                {filteredSignals.map((signal) => (
                  <SignalRow key={signal.ingested_at} signal={signal} />
                ))}
