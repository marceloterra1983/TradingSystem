...
import { MessageSquare, Eye, AlertCircle, Image as ImageIcon, RefreshCw, Search } from 'lucide-react';
...
async function fetchForwardedMessages(limit: number): Promise<ForwardedMessage[]> {
  const response = await fetch(`${TP_CAPITAL_API_URL}/forwarded-messages?limit=${limit}`);
  if (!response.ok) {
...
export function ForwardedMessagesTable() {
  ...
  const channelOptions = useMemo(() => {
    const channels = new Set(messages.map(m => m.source_channel_name || `Canal ${m.source_channel_id}`));
  ...
  const filteredMessages = useMemo(() => {
    return messages.filter((msg) => {
      const matchesChannel = channelFilter === 'all' || 
        (msg.source_channel_name || `Canal ${msg.source_channel_id}`) === channelFilter;
      ...
      const matchesSearch = searchTerm
        ? (msg.message_text || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
          (msg.source_channel_name || '').toLowerCase().includes(searchTerm.toLowerCase())
        : true;
    ...
    <>
      <CollapsibleCard cardId="forwarded-messages-table">
        ...
        <div className="mb-4 grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          {(channelFilter !== 'all' || mediaFilter !== 'all' || searchTerm) && (
            <Button
              variant="outline"
              size="sm"
              onClick={() => {
        ...
        {isLoading ? (
          ...
          <div className="flex items-center justify-center py-12">
            <div className="flex flex-col items-center gap-2 text-gray-500">
              ...
              <p className="text-sm font-medium">Nenhuma mensagem encontrada com os filtros aplicados</p>
              <Button
                variant="outline"
                size="sm"
                onClick={() => {
          ...
          <div className="flex items-center justify-center py-12">
            <div className="flex flex-col items-center gap-2 text-gray-500">
              <MessageSquare className="h-12 w-12 opacity-20" />
              <p className="text-sm font-medium">Nenhuma mensagem encaminhada ainda</p>
                ...
                {filteredMessages.map((message: ForwardedMessage) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      ...
                      >
                        {message.forward_method === 'copy' ? 'Cópia' : 'Encaminhar'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <Button
