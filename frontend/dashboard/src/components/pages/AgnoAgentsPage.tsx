...
import { Activity, BrainCircuit, BookOpen, ExternalLink, GaugeCircle, ListTree, Radar, Rocket, ShieldCheck, Workflow } from 'lucide-react';
...
const ENV_DEFAULT_BASE_URL = sanitizeBaseUrl(import.meta.env.VITE_AGNO_AGENTS_URL);
  ...
  const sections = useMemo(
    () => [
          ...
          <CollapsibleCard cardId="agno-overview" defaultCollapsed={false}>
            ...
            <CollapsibleCardContent>
              <div className="flex flex-wrap items-center gap-3 mb-4">
                <Badge variant="outline">{(() => { try { return new URL(baseUrl).host; } catch { return baseUrl; } })()}</Badge>
              ...
              <div className="rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-3 flex flex-col gap-2">
                <div className="text-sm text-slate-700 dark:text-slate-300">Base URL do Agno Agents</div>
                <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_auto_auto_auto] gap-2 items-center">
                  <Input value={tmpUrl} onChange={(e) => setTmpUrl(e.target.value)} placeholder="http://localhost:8211" />
                  <Button variant="outline" onClick={() => setTmpUrl(baseUrl)}>Reset</Button>
                  <Button variant="outline" onClick={async () => {
                    setPinging(true); setPingOk(null); setPingText('');
                    try {
                      const url = sanitizeBaseUrl(tmpUrl);
                      const res = await fetch(`${url}/health`);
                      if (!res.ok) { setPingOk(false); setPingText(`HTTP ${res.status}`); }
                      else { setPingOk(true); setPingText('OK'); }
                    } catch (e: any) { setPingOk(false); setPingText(e?.message || 'Network error'); }
                    finally { setPinging(false); }
                  }}>Testar</Button>
                  <Button onClick={async () => { setBaseUrl(sanitizeBaseUrl(tmpUrl));
                  ...
                  <div className="text-xs">
                    {pinging && <span className="text-slate-500">Testando…</span>}
                    {!pinging && pingOk === true && <span className="text-emerald-600">{pingText || 'OK'}</span>}
                    {!pinging && pingOk === false && <span className="text-red-600">{pingText || 'Erro'}</span>}
                  </div>
                </div>
                <div className="text-xs text-slate-500">Persistido em localStorage (chave <code className="font-mono">agno.baseUrl</code>).</div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/60 p-4 space-y-2">
                  ...
                  <ul className="list-disc list-inside text-sm text-slate-600 dark:text-slate-400 space-y-1">
                    <li>MarketAnalysisAgent — correlação TP Capital + Workspace.</li>
      ...
      },
      {
        ...
        content: (
          <AgentsRegistrySection baseUrl={baseUrl} />
        ),
      },
    ],
...
function AgentsRegistrySection({ baseUrl }: { baseUrl: string }) {
  ...
  function openRunDialog(agent: AgentDescriptor) {
    ...
    const modelDefault = (agent.args || []).find(a => a.name === 'model')?.resolvedDefault as string | undefined;
    const maxDefault = (agent.args || []).find(a => a.name === 'maxContext')?.resolvedDefault as number | undefined;
    const outDirDefault = (agent.args || []).find(a => a.name === 'outDir')?.resolvedDefault as string | undefined;
    setForm({
      since: sinceDefault,
      dry: false,
      model: modelDefault || '',
  ...
  async function submitRun() {
    ...
    try {
      ...
      if (form.since && form.since !== 'local-midnight') payload.since = form.since;
  ...
  return (
    <CollapsibleCard cardId="agno-agents-registry" defaultCollapsed={false}>
      <CollapsibleCardHeader>
        ...
        <CollapsibleCardDescription>
          Lista dos agentes locais com papel, instruções, variáveis e argumentos.
        </CollapsibleCardDescription>
      </CollapsibleCardHeader>
      <CollapsibleCardContent>
        {loading && (
        ...
        {!loading && !error && agents.length === 0 && (
          <div className="text-sm text-slate-500">Nenhum agente encontrado.</div>
        )}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {agents.map((agent) => (
            <div
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  <div className="font-semibold text-slate-900 dark:text-slate-100">{agent.name} <span className="text-slate-500 text-xs">({agent.id})</span></div>
                  {agent.role && (
                    <div className="text-xs text-slate-500 dark:text-slate-400">{agent.role}</div>
                  )}
                </div>
                <div className="flex gap-2">
                  {agent.runtime && <Badge variant="outline">{agent.runtime}</Badge>}
                  <Button
                    size="sm"
                    onClick={() => openRunDialog(agent)}
                    disabled={agent.id !== 'docusaurus-daily'}
                    className="gap-2"
                    title={agent.id !== 'docusaurus-daily' ? 'Execução por UI disponível inicialmente apenas para docusaurus-daily' : 'Executar agora'}
                  >
              ...
              <div className="text-xs text-slate-500 dark:text-slate-400">
                {agent.entry && (
                  <div><span className="font-mono">entry:</span> {agent.entry}</div>
                )}
                {agent.env && agent.env.length > 0 && (
                  <div className="mt-1"><span className="font-mono">env:</span> {agent.env.join(', ')}</div>
                )}
              </div>

              {agent.args && agent.args.length > 0 && (
                <div className="mt-2">
                  <div className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Argumentos</div>
                  <div className="space-y-1">
                    {agent.args.map((a) => (
                      <div key={a.name} className="text-xs text-slate-700 dark:text-slate-300 flex flex-col border border-slate-100 dark:border-slate-800 rounded p-2">
                        <div className="flex items-center justify-between">
                          <span className="font-mono">--{a.name}</span>
                          <Badge variant="outline">{a.type}</Badge>
                        </div>
                        {a.description && <div className="text-slate-500 mt-1">{a.description}</div>}
                        <div className="mt-1 grid grid-cols-1 md:grid-cols-2 gap-2">
                          {'default' in a && (
                            <div className="text-slate-500">
                              <span className="font-mono">default:</span> <span className="break-all">{String(a.default)}</span>
                            </div>
                          )}
                          {'resolvedDefault' in a && a.resolvedDefault !== undefined && (
                            <div className="text-slate-500">
                              <span className="font-mono">resolved:</span> <span className="break-all">{String(a.resolvedDefault)}</span>
                            </div>
                          )}
                        </div>
                      </div>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <Input value={form.since} onChange={(e) => setForm(f => ({...f, since: e.target.value}))} placeholder="local-midnight ou 2025-10-27 00:00" />
                </div>
                <div className="flex items-center gap-2 mt-6">
                  <Checkbox checked={form.dry} onCheckedChange={(v) => setForm(f => ({...f, dry: !!v}))} />
                  <span className="text-xs text-slate-600">dry (não escreve arquivos)</span>
                </div>
                <div>
                  <label className="text-xs text-slate-500">model</label>
                  <Input value={form.model} onChange={(e) => setForm(f => ({...f, model: e.target.value}))} placeholder="ex.: llama3.1:8b" />
                </div>
                <div>
                  <label className="text-xs text-slate-500">maxContext</label>
                  <Input type="number" value={form.maxContext} onChange={(e) => setForm(f => ({...f, maxContext: e.target.value}))} placeholder="ex.: 20000" />
                </div>
                <div className="flex items-center gap-2">
                  <Checkbox checked={form.noValidate} onCheckedChange={(v) => setForm(f => ({...f, noValidate: !!v}))} />
                  <span className="text-xs text-slate-600">noValidate</span>
                </div>
                <div>
                  <label className="text-xs text-slate-500">outDir</label>
                  <Input value={form.outDir} onChange={(e) => setForm(f => ({...f, outDir: e.target.value}))} placeholder="docs/content/reports/daily" />
                </div>
              </div>
              {errorMsg && <div className="text-sm text-red-600">{errorMsg}</div>}
              {result && <div className="text-sm text-emerald-600 break-all">{result}</div>}
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setOpenId(null)} disabled={submitting}>Cancelar</Button>
