...
describeIntegration('Dashboard ↔ API integration', () => {
  ...
  it(
    'validates documentation metrics contract',
    async () => {
      const { documentationService } = await import('../../services/documentationService');
  ...
  it(
    'confirms workspace API health via endpoint validator',
    async () => {
      // Re-import with current env overrides to pick up workspace base URL.
      const { ENDPOINTS, validateEndpoint } = await import('../../config/endpoints');
