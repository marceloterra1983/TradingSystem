...
export function preloadHints(options = {}) {
    const { chunks = ['react-vendor', 'ui-radix', 'icons-vendor', 'utils-vendor'], modulepreload = true, } = options;
    return {
        ...
        transformIndexHtml(html, ctx) {
            ...
            Object.values(ctx.bundle).forEach((chunk) => {
                if (chunk.type === 'chunk') {
                    ...
                    const shouldPreload = chunks.some(name => chunkName.startsWith(name));
                    if (shouldPreload) {
                        const rel = modulepreload ? 'modulepreload' : 'preload';
                        preloadLinks.push(`<link rel="${rel}" href="/${chunk.fileName}" as="script" crossorigin>`);
                    }
                }
            });
