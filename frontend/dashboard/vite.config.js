...
const createRewrite = (pattern, basePath) => {
  ...
  return (incomingPath) => {
    ...
    const basePrefix = normalizedBase.startsWith("/")
      ? normalizedBase
      : `/${normalizedBase}`;
...
export default defineConfig(({ mode }) => {
  ...
  if (mode === "development") {
    ...
    console.log(
      "[vite] TELEGRAM_GATEWAY_API_URL=",
      env.VITE_TELEGRAM_GATEWAY_API_URL
    );
    console.log(
      "[vite] TELEGRAM_GATEWAY_API_TOKEN=",
      env.VITE_TELEGRAM_GATEWAY_API_TOKEN
    );
    console.log("[vite] API_SECRET_TOKEN=", env.VITE_API_SECRET_TOKEN);
  }
  const isProd = mode === "production";
  const defaultDashboardPort = Number(
    env.VITE_DASHBOARD_PORT || env.VITE_GATEWAY_PORT || 9082
  );
  const dashboardPort = defaultDashboardPort;
  ...
  const tpCapitalProxy = resolveProxy(
    env.TP_CAPITAL_PROXY_TARGET ||
      env.VITE_TP_CAPITAL_PROXY_TARGET ||
      env.VITE_TP_CAPITAL_API_URL,
    "http://localhost:4008"
  );
  // Docs API (FlexSearch + CRUD) runs on 3405; 3400 serves Docusaurus dev/NGINX
  const docsApiProxy = resolveProxy(
    env.DOCS_API_PROXY_TARGET ||
      env.VITE_DOCS_API_PROXY_TARGET ||
      env.VITE_DOCS_API_URL,
    "http://localhost:3405"
  );
  ...
  const docsProxy = resolveProxy(
    env.DOCUSAURUS_PROXY_TARGET ||
      env.VITE_DOCUSAURUS_PROXY_TARGET ||
  ...
  const mcpProxy = resolveProxy(
    env.VITE_MCP_PROXY_TARGET,
    "http://localhost:3847"
  );
  ...
  // Note: n8nBasePath reserved for future n8n path configuration
  // const _n8nBasePath = (() => {
  ...
  const n8nBasicAuthHeader = (() => {
    ...
    const n8nPass =
      ...
      env.VITE_N8N_BASIC_AUTH_PASSWORD ||
      "changeme";
    if (mode === "development") {
      console.log("[vite] n8n basic auth user", n8nUser);
      console.log(
        "[vite] n8n basic auth password present",
        typeof n8nPass === "string" && n8nPass.length > 0
      );
    }
    if (!n8nUser || !n8nPass) {
      return null;
  ...
  const stripFrameBlockingHeaders = (proxy) => {
    proxy.on("proxyRes", (proxyRes) => {
      ...
      const cspHeader = resolveHeaderValue(
      ...
      const sanitized = cspHeader
        ...
        .filter(
          (directive) =>
            directive.length > 0 && !/frame-ancestors/i.test(directive)
        )
        .join("; ");
      if (sanitized) {
  ...
  const preserveProxyLocation = (proxy, mountPath) => {
    ...
    proxy.on("proxyRes", (proxyRes) => {
      ...
      proxyRes.headers["location"] = normalizedMount
        ? `${normalizedMount}${ensured}`.replace(/\/{2,}/g, "/")
        : ensured;
    });
  };
      ...
      proxy: {
        ...
        "/api/v1/rag/collections": {
          ...
          rewrite: createRewrite(
            ...
            ragCollectionsProxy.basePath
              ? `${ragCollectionsProxy.basePath}/api/v1/rag/collections`
              : "/api/v1/rag/collections"
          ),
        },
        "/api/v1/rag/directories": {
          ...
          rewrite: createRewrite(
            /^\/api\/v1\/rag\/directories/,
            ragCollectionsProxy.basePath
              ? `${ragCollectionsProxy.basePath}/api/v1/rag/directories`
              : "/api/v1/rag/directories"
          ),
        },
        "/api/v1/rag/models": {
          ...
          rewrite: createRewrite(
            /^\/api\/v1\/rag\/models/,
            ragCollectionsProxy.basePath
              ? `${ragCollectionsProxy.basePath}/api/v1/rag/models`
              : "/api/v1/rag/models"
          ),
        },
        "/api/v1/rag/ingestion": {
          ...
          rewrite: createRewrite(
            /^\/api\/v1\/rag\/ingestion/,
            ragCollectionsProxy.basePath
              ? `${ragCollectionsProxy.basePath}/api/v1/rag/ingestion`
              : "/api/v1/rag/ingestion"
          ),
        },
        ...
        "/api/v1/rag": {
          ...
          rewrite: (incomingPath) => {
            ...
            const remainder = stripped.startsWith("/")
              ? stripped
              : `/${stripped}`;
            if (!ragCollectionsProxy.basePath) {
              return (
                `/api/v1/rag${remainder}`.replace(/\/+/g, "/") || "/api/v1/rag"
              );
            }
            const base = ragCollectionsProxy.basePath.startsWith("/")
              ...
              : `/${ragCollectionsProxy.basePath}`;
            return (
              `${base}/api/v1/rag${remainder}`.replace(/\/+/g, "/") ||
              `${base}/api/v1/rag`
            );
          },
        },
