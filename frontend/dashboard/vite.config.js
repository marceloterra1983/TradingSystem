...
export default defineConfig(({ mode }) => {
    ...
    const docsProxy = resolveProxy(env.DOCUSAURUS_PROXY_TARGET || env.VITE_DOCUSAURUS_PROXY_TARGET || env.VITE_DOCUSAURUS_URL, 'http://localhost:3404');
    const dbUiPgAdminProxy = resolveProxy(env.DB_UI_PGADMIN_PROXY_TARGET || env.VITE_DB_UI_PGADMIN_PROXY_TARGET, 'http://localhost:5050');
    const dbUiPgWebProxy = resolveProxy(env.DB_UI_PGWEB_PROXY_TARGET || env.VITE_DB_UI_PGWEB_PROXY_TARGET, 'http://localhost:8081');
    const dbUiAdminerProxy = resolveProxy(env.DB_UI_ADMINER_PROXY_TARGET || env.VITE_DB_UI_ADMINER_PROXY_TARGET, 'http://localhost:8082');
    const dbUiQuestDbProxy = resolveProxy(env.DB_UI_QUESTDB_PROXY_TARGET || env.VITE_DB_UI_QUESTDB_PROXY_TARGET, 'http://localhost:9002');
    const firecrawlProxy = resolveProxy(env.VITE_FIRECRAWL_PROXY_TARGET || env.VITE_FIRECRAWL_PROXY_URL, 'http://localhost:3600');
    ...
    return {
        ...
        define: {
            ...
            'import.meta.env.VITE_TELEGRAM_GATEWAY_API_URL': JSON.stringify(env.VITE_TELEGRAM_GATEWAY_API_URL || env.VITE_API_BASE_URL || ''),
