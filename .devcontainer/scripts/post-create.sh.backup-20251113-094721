...
echo "🚀 TradingSystem Dev Container - Post-Create Setup"
echo "================================================="
...
echo -e "\n${BLUE}📦 Installing documentation dependencies...${NC}"
cd docs
npm install --prefer-offline --no-audit --legacy-peer-deps
cd ..
...
npm run env:validate || echo -e "${YELLOW}⚠️  Environment validation failed. Please check .env file.${NC}"

# 8. Build TypeScript projects
echo -e "\n${BLUE}🏗️  Building TypeScript projects...${NC}"
npm run type-check || echo -e "${YELLOW}⚠️  Type checking found some issues. You can fix them later.${NC}"

# 9. Generate command database (if available)
echo -e "\n${BLUE}📋 Generating commands database...${NC}"
npm run commands:generate || echo -e "${YELLOW}⚠️  Commands generation failed. Non-critical.${NC}"

# 10. Generate agents directory (if available)
echo -e "\n${BLUE}🤖 Generating agents directory...${NC}"
npm run agents:generate || echo -e "${YELLOW}⚠️  Agents generation failed. Non-critical.${NC}"

# 11. Success message
echo -e "\n${GREEN}✅ Post-create setup completed successfully!${NC}"
echo -e "\n${BLUE}📚 Next steps:${NC}"
echo -e "  1. Update .env file with your actual values"
echo -e "  2. Run ${GREEN}npm run start${NC} to start all services"
echo -e "  3. Access Dashboard at http://localhost:8090"
