...
cd tools/compose
docker compose -f docker-compose.telegram.yml up -d telegram-pgbouncer --force-recreate
```
...
**Fix**: Update `docker-compose.telegram.yml`:
```yaml
environment:
...
**Fix**: Update `docker-compose.tp-capital-stack.yml`:
```yaml
environment:
  # ❌ WRONG
...
**Fix**: Update `docker-compose.dashboard.yml`:
```yaml
environment:
...
# 3. Test proxy
curl http://localhost:3103/api/tp-capital/health
...
docker compose -f docker-compose.tp-capital-stack.yml up -d
...
- **Policies**: 
  - [POL-0003 Addendum 001: Port Mapping Rules](/governance/policies/addendums/POL-0003-ADDENDUM-001-port-mapping-rules.md)
  - [POL-0002 Addendum 001: Empty Value Validation](/governance/policies/addendums/POL-0002-ADDENDUM-001-empty-value-validation.md)
- **Checklists**: [Pre-Deployment Checklist](/governance/controls/PRE-DEPLOY-CHECKLIST.md)
- **Scripts**: 
