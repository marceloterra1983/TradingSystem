...
- Disponível via gateway em: `${GATEWAY_PUBLIC_URL}/apps/course-crawler` (padrão `http://localhost:9082/apps/course-crawler`)
- Fallback direto para desenvolvimento: `${VITE_COURSE_CRAWLER_APP_URL}` (`http://localhost:4201`)
...
- Dashboard (iframe): `http://localhost:9082/#/course-crawler` (iframe aponta para `/apps/course-crawler`)
...
| `VITE_COURSE_CRAWLER_GATEWAY_PATH` | Caminho servido pelo Traefik (default `/apps/course-crawler`) |
