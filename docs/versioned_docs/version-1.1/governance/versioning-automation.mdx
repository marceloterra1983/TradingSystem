...
Follow the manual steps documented in [`versioning-guide`](./versioning-guide) when automation is unavailable. The automated script implements the same 10-step process programmatically and can be executed locally for preview (`--dry-run`) or manual commits (`--auto-commit` omitted).
...
- [`versioning-guide`](./versioning-guide) – Manual versioning process.
- [`ci-cd-integration`](./ci-cd-integration) – Overview of documentation workflows.
- [`validation-guide`](./validation-guide) – Frontmatter and maintenance validation.
