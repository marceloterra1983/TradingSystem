...
1. **LlamaIndex** – Handles document ingestion, chunking, metadata enrichment, and query serving.
2. **Ollama** – Provides local LLM inference for both ingestion and query flows.

## 🧱 Architecture Overview

```
┌────────────────────────┐      ┌────────────────────────┐
│   LlamaIndex Ingest    │      │   LlamaIndex Query     │
│ (FastAPI + Workers)    │      │  (FastAPI + Adapters)  │
└────────────┬───────────┘      └────────────┬───────────┘
             │                               │
             └───────────────┬───────────────┘
                             ▼
...
- **Transport:** FastAPI services exposed via `docker-compose.rag.yml`.
...
```
tools/llamaindex/
├── Dockerfile.ingestion     # Ingestion service image (FastAPI worker)
├── Dockerfile.query         # Query service image (FastAPI endpoint)
├── src/
│   ├── ingestion/           # Watchers, chunkers, metadata enrichers
│   ├── query/               # REST APIs, aggregations, similarity search
│   ├── pipelines/           # Shared orchestration utilities
│   └── settings.py          # Centralized configuration
└── scripts/
    ├── bootstrap.sh         # Creates collections, seeds metadata
    └── sync.sh              # Sync helper for local datasets
```
...
- Configured through `tools/compose/docker-compose.rag.yml` (ports 11434/11435).
...
- `scripts/rag/sync-collections.sh` – Syncs docs from `docs/content/` to the ingestion folder.
- `scripts/rag/test-query.sh` – Smoke test against the query API.
- `scripts/rag/reset.sh` – Drops Qdrant collections and recreates them from scratch.
