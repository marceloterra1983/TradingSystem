...
- `LLAMAINDEX_FORCE_GPU` / `LLAMAINDEX_GPU_NUM` – force GPU execution and set GPU count per request.
- `LLAMAINDEX_GPU_MAX_CONCURRENCY` – cap concurrent GPU jobs (defaults to `1`).
- `LLAMAINDEX_GPU_COOLDOWN_SECONDS` – optional pause between jobs.
- `LLAMAINDEX_GPU_USE_FILE_LOCK` / `LLAMAINDEX_GPU_LOCK_PATH` – cross-process lock consumed by Python services and Node scripts.
