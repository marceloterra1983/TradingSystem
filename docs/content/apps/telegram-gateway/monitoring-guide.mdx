...
slug: /apps/telegram-gateway/monitoring-guide
sidebar_position: 11
...
- **Replication Lag** - Master → Replica delay (target: &lt;50ms)
...
**Troubleshooting:**
- Hit rate &lt;50% → Increase TTL or memory
- Eviction rate high → Increase maxmemory
- Replication lag >1s → Check network or replica resources
...
#    - Dependency unavailable → Check Redis, PgBouncer
#    - Process crash → Check for OOM, errors in logs
...
# Edit: tools/compose/docker-compose.4-2-telegram-stack.yml
