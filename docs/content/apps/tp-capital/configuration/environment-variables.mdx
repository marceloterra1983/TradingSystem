...
slug: /apps/tp-capital/configuration/environment-variables
...
**Important Notes:**
- ✅ **Always use root `.env`** - All services load from project root
- ✅ **VITE_ variables require rebuild** - Changes need `docker compose up -d --build`
- ✅ **No VITE_ prefix for proxy targets** - Server-side only (see [PROXY-BEST-PRACTICES.md](../../../frontend/engineering/PROXY-BEST-PRACTICES.md))
...
**Security Notes:**
- 🔒 **Password Strength**: Minimum 32 characters, use `openssl rand -hex 32`
- 🔒 **No Host Exposure**: Port 5432 is internal only (no port mapping to host)
- 🔒 **Connection Pooling**: PgBouncer currently disabled (SCRAM-SHA-256 auth issues)
...
- 🔒 **Generate Strong Keys**: `openssl rand -hex 32` (produces 64-char hex)
- 🔒 **No Direct Access**: API only accessible via Gateway (Traefik routing)
- 🔒 **HTTPS in Production**: Enable TLS at Gateway level (future)
...
**⚠️ CRITICAL: VITE Variables Require Rebuild!**
```bash
...
# ❌ WRONG - Do NOT use VITE_ prefix for proxy targets!
# VITE_TP_CAPITAL_API_URL=http://tp-capital-api:4005  # Browser cannot resolve!

# ✅ CORRECT - Server proxies, browser uses relative paths
...
Browser → GET /api/tp-capital/signals
         ↓ (Vite proxy)
Container → GET http://api-gateway:9080/api/tp-capital/signals
         ↓ (Traefik StripPrefix)
API → GET /signals (receives without prefix)
```
...
- **Proxy Best Practices**: [PROXY-BEST-PRACTICES.md](../../../frontend/engineering/PROXY-BEST-PRACTICES.md)
- **Troubleshooting**: [TP Capital Connectivity Issues](../runbooks/troubleshooting-connectivity.mdx)
