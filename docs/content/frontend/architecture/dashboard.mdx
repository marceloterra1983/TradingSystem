...
slug: /frontend/architecture/dashboard
sidebar_position: 1
...
- **Port:** 9080 (exposto pelo Traefik; 3103 permanece como legado para casos isolados)
...
```
Dashboard (Port 9080 via Traefik)
├── 1. Apps (Cyan)
...
│   ├── RAG Services → LlamaIndexPage
│   ├── Kestra → KestraPage
│   ├── n8n Automations → N8nPage
│   └── Ports → PortsPage
│
├── 4. Connections
...
**Dev Server:** `http://localhost:9080` (Docker container; 3103 permanece legado apenas para troubleshooting)
...
```typescript
const workspaceApiUrl = import.meta.env.VITE_WORKSPACE_API_URL;
```
