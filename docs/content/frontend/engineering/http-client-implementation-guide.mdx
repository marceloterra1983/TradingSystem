...
slug: /frontend/engineering/http-client-implementation-guide
sidebar_position: 5
...
- 🏛️ [ADR-003: API Gateway Implementation](../../reference/adrs/ADR-003-api-gateway-implementation.md)
  ...
  private setupInterceptors(enableLogging?: boolean) {
    ...
    this.client.interceptors.response.use(
      ...
      (error: AxiosError) => {
        ...
        console.error(`[HttpClient] ✗ ${error.config?.url} (${duration}ms)`, error.message);
...
export class WorkspaceClient {
  ...
  constructor() {
    this.http = new HttpClient({
      baseURL: 'http://localhost:9082/api/workspace', // ✅ Via Traefik
...
export function TelegramGatewayPage() {
  ...
  async function handleSyncMessages() {
    ...
    if (!serviceHealth?.healthy) {
      alert('Serviço Telegram Gateway está indisponível. Tente novamente em alguns instantes.');
      return;
    }
    ...
    try {
      ...
      alert(`✅ ${result.data.totalMessagesSynced} mensagem(ns) recuperada(s)!`);
    } catch (error) {
...
**Solutions:**
1. **Backend is actually down** → Fix backend service
2. **Timeout too aggressive** → Increase timeout for operation type
3. **Failure threshold too low** → Increase from 5 to 10 failures
...
**Solutions:**
1. **Backend optimization needed** → Profile backend API
2. **Network latency** → Check Docker network performance
3. **Database query slow** → Add indexes, optimize queries
...
- 🏛️ [API Gateway Policy](https://github.com/marceloterra1983/TradingSystem/blob/main/governance/policies/api-gateway-policy.md)
