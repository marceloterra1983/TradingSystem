...
domain: tools
type: guide
summary: "Fluxo de trabalho diário no Dev Container: startup, desenvolvimento, debugging, testing, commits e troubleshooting"
status: active
...
Este guia descreve o **fluxo de trabalho completo** para desenvolvedores usando o Dev Container, desde o startup até o commit de código.
...
- ✅ Detecta `.devcontainer/devcontainer.json`
- ✅ Mostra notificação: "Reopen in Container"
...
**Primeira vez do dia:**
- ⏱️ ~30 segundos (container já existe)
- ✅ VS Code Server inicia
- ✅ Extensions carregam
- ✅ Terminal abre automaticamente **dentro do container**
...
**Opção 1: Universal Startup (Recomendado)**
```bash
# Dentro do Dev Container
start

# Output:
# 🚀 Starting TradingSystem...
# ✅ Docker containers started
# ✅ Node.js services started
# 🎉 All services healthy!
```

**Opção 2: Granular Startup**
```bash
...
# Modo mínimo (essenciais apenas)
start-minimal
```

**Services iniciados:**
- ✅ Traefik Gateway (9082, 9083)
- ✅ Dashboard UI (3103) - Production build
- ✅ Workspace API (3200)
- ✅ TP Capital API (4005)
- ✅ Telegram Gateway (4010)
- ✅ Docs Hub (80)
...
# ✅ Workspace API: Healthy (8ms)
# ✅ TP Capital API: Healthy (12ms)
# ✅ Telegram Gateway: Healthy (4ms)
# ✅ workspace-db: Up (2 connections)
# ✅ workspace-redis: Up (Hit rate: 79.4%)
```
...
**Hot Reload ativo:**
- ✅ Alterações em `.tsx` refletem instantaneamente
- ✅ Alterações em `.css` aplicadas sem reload
- ✅ State preservado (Fast Refresh)
...
### Quanto tempo leva o workflow diário?
...
### Preciso rebuild toda vez que faço push?

**Não!** Rebuild é necessário apenas quando:
- Modificar `.devcontainer/*` files
- Adicionar system packages
- Atualizar Node.js/Python version

**Código normal (JS/TS/React):** Sem rebuild necessário.

### Como trabalhar offline?

**Dev Container funciona offline:**
- ✅ Container já está baixado
- ✅ Dependencies já instaladas
- ✅ Pode desenvolver normalmente
- ❌ Não pode push para GitHub (obviously)
- ❌ Não pode pull latest

**Services externos (Telegram API):** Não funcionam offline.
...
### Como compartilhar configuração com a equipe?
...
**NÃO committar:**
- ❌ `.env` (secrets)
- ❌ `.devcontainer/.env.local` (user-specific)
- ❌ `node_modules`
- ❌ `.vscode/settings.json` (user preferences)
