...
1. **Traefik API Gateway** - Reverse proxy para produção (containers Docker)
2. **Vite Dev Server Proxy** - Proxy de desenvolvimento (hot reload)

Cada um tem **propósitos e configurações específicas**.
...
**Traefik** é um **reverse proxy e load balancer** moderno que:
- ✅ Descobre serviços automaticamente (Docker labels)
- ✅ Roteia requests baseado em rules (path, host, headers)
- ✅ Aplica middlewares (CORS, rate limit, compression, circuit breaker)
- ✅ Expõe métricas Prometheus
- ✅ Possui dashboard de monitoramento
- ✅ Suporta múltiplos backends (Docker, Kubernetes, Consul, etc)
...
│  ┌───────────────────────────────────────────────────┐ │
│  │  Entrypoint: web (9080 internal, 9082 external)  │ │
│  └───────────────────────────────────────────────────┘ │
│                        ↓                                │
│  ┌───────────────────────────────────────────────────┐ │
│  │  Routers (priority-based)                         │ │
│  │  ├─> /api/workspace → Priority 100                │ │
│  │  ├─> /api/telegram-gateway → Priority 95          │ │
│  │  ├─> /api/tp-capital → Priority 90                │ │
│  │  ├─> /api/docs → Priority 85                      │ │
│  │  ├─> /docs → Priority 50                          │ │
│  │  └─> / → Priority 1                               │ │
│  └───────────────────────────────────────────────────┘ │
│                        ↓                                │
│  ┌───────────────────────────────────────────────────┐ │
│  │  Middlewares Chain                                │ │
│  │  ├─> CORS (localhost:5173, localhost:9082)        │ │
│  │  ├─> Rate Limit (100 req/min per IP)              │ │
│  │  ├─> Compression (gzip/brotli, >1KB)              │ │
│  │  ├─> Circuit Breaker (20% error → open)           │ │
│  │  └─> Security Headers (X-Frame-Options, XSS)      │ │
│  └───────────────────────────────────────────────────┘ │
│                        ↓                                │
│  ┌───────────────────────────────────────────────────┐ │
...
Traefik usa **Docker labels** para descobrir serviços automaticamente.
...
Matches:
- ✅ `/api/workspace/items`
- ✅ `/api/workspace/items/123`
- ✅ `/api/workspace/health`
- ❌ `/api/workspace-extra` (não começa com `/api/workspace/`)

**Host (Para múltiplos domínios):**
```yaml
...
- Se request match múltiplas rules, usa a de maior prioridade
...
**Por que essa ordem?**
- ✅ Rotas específicas (`/api/workspace`) antes de genéricas (`/`)
- ✅ APIs (100-85) antes de UIs (50-1)
- ✅ Catch-all (`/`) por último (default route)
...
Traefik (sem rewrite) → workspace-api:3200/api/workspace/items
Workspace API expects → workspace-api:3200/api/items
...
Traefik (stripprefix) → /items
Workspace API → Handles /items correctly
```
...
**Use case:**
- ✅ API backend está down → Circuit breaker abre
- ✅ Requests retornam 503 Service Unavailable (rápido)
- ✅ Evita sobrecarregar API já problemática
...
**Ordem de execução:**
```
Request → CORS → Rate Limit → Compress → Circuit Breaker → Security Headers → Backend
```

### Health Checks

**Traefik auto-detecta health de serviços:**

**Configuração (Workspace API):**
```yaml
services:
  workspace-api:
    healthcheck:
...
**Traefik behavior:**
- ✅ Se health check **passa** → Roteie requests
- ❌ Se health check **falha** → Marque como unhealthy (não roteia)
- ⚠️ Se **todos backends unhealthy** → 503 Service Unavailable

**Dashboard:** http://localhost:9083/dashboard/ mostra health status de todos os serviços.
...
**Features:**
- ✅ Visualizar routers (rules, priorities, middlewares)
- ✅ Visualizar services (backends, health status)
- ✅ Visualizar middlewares (configurações)
- ✅ Visualizar entrypoints (portas expostas)
- ✅ Métricas em tempo real (requests/s, errors/s)
- ✅ Access logs (last 100 requests)

**Debugging workflow:**
1. Abrir dashboard
2. HTTP → Routers → Encontrar seu router
3. Verificar rule, priority, middlewares
4. Clicar em Service → Ver health status
5. Verificar se requests estão chegando (metrics)
...
- ✅ Intercepta requests no dev server
- ✅ Forwards para backend services
- ✅ Mantém hot reload funcionando
- ✅ Evita CORS issues (same-origin)
- ✅ Permite usar paths relativos no código
...
# ✅ OK - Safe to expose (browser uses this)
VITE_API_BASE_URL=http://localhost:9082
VITE_ENVIRONMENT=development
```

**Why no `VITE_` prefix?**
- ❌ Vite expõe `VITE_*` variables ao browser bundle
- ❌ Browser tenta conectar em `http://workspace-api:3200` (falha)
- ❌ Revela topologia interna da rede
- ✅ Server-side variables ficam apenas no Vite dev server
...
Vite Proxy detects: /api/workspace
         ↓
Rewrite: /api/workspace/items → /api/items
         ↓
...
```
Browser → http://localhost:5173/api/tp-capital/signals
         ↓
  ...
  '/api/workspace': {
    ...
    configure: (proxy, options) => {
      proxy.on('proxyReq', (proxyReq, req, res) => {
        ...
        console.log(`[Vite Proxy] ${req.method} ${originalUrl} → ${targetUrl}`);
      });
...
**Output:**
```
[Vite Proxy] GET /api/workspace/items → http://workspace-api:3200/api/items
...
Steps:
1. HTTP → Routers → Find your router
2. Verify rule, priority, middlewares
3. Click Service → Check health status
4. HTTP → Services → See backend servers
```
...
### Issue 1: "API Indisponível" - Vite Proxy
...
**Root Cause:** CORS middleware não aplicado
...
3. Circuit breaker volta para "Half-Open" → "Closed"
...
- **[API Access](./api-access.mdx)** - Padrões de acesso a APIs
...
- **[API Gateway Policy](/governance/policies/api-gateway-policy)** - Traefik governance

### Frontend Engineering
- **[Proxy Best Practices](/frontend/engineering/proxy-best-practices)** - Guia completo
...
### Qual a diferença entre Traefik e Vite Proxy?

**Traefik:**
- ✅ Production-ready reverse proxy
- ✅ Múltiplas backends, load balancing
- ✅ Middlewares (CORS, rate limit, etc)
- ✅ Health checks, circuit breakers
- ✅ Métricas Prometheus
- ⚠️ Requer rebuild para mudanças
...
- ✅ Fast feedback loop
- ⚠️ Sem middlewares avançados
- ⚠️ Apenas para dev mode

### Posso usar ambos ao mesmo tempo?

**Sim!** É exatamente o que fazemos:

```
Development: Browser → Vite Proxy → APIs
Production: Browser → Traefik Gateway → APIs
```

Vite automaticamente desativa proxy em production build.

### Como adicionar um novo serviço ao Traefik?
...
→ HTTP → Routers → "newservice" deve aparecer
```
