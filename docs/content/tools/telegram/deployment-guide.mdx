...
slug: /tools/telegram/deployment-guide
description: Guia completo para deploy e operação da stack Telegram com monitoramento integrado
tags:
  - telegram
  - deployment
  - docker
...
domain: tools
type: guide
summary: Deploy e operação da stack Telegram completa (12 containers) com core services e monitoring integrado
status: active
...
A **Telegram Stack** é uma solução completa para captura, processamento e armazenamento de mensagens do Telegram, com monitoramento integrado.
...
│  ├── TimescaleDB (internal)    - Banco de dados time-series   │
│  ├── PgBouncer (internal)      - Connection pooling           │
│  ├── Redis Master (internal)   - Cache principal              │
│  ├── Redis Replica (internal)  - Read replica                 │
│  ├── Redis Sentinel (internal) - HA monitoring                │
│  ├── RabbitMQ (5672/15672)     - Message broker               │
│  ├── MTProto (14007)           - Telegram client              │
│  └── Gateway API (Traefik → /api/telegram-gateway) - REST API │
│                                                               │
│  MONITORING SERVICES (4 containers)                          │
│  ├── Prometheus (internal)     - Metrics collection           │
│  ├── Grafana (internal)        - Visualization                │
│  ├── Postgres Exporter (internal) - Database metrics          │
│  └── Redis Exporter (internal) - Cache metrics                │
│                                                               │
└─────────────────────────────────────────────────────────────┘
```
...
```bash
# 1. Navegar para o diretório compose
...
- **Porta:** 5434 (interna), não exposta externamente
...
- **Porta:** 6434 (interna), não exposta externamente
- **Pool Size:** 50 conexões
- **Health Check:** `pg_isready`
- **Recursos:** 0.5 CPU, 256MB RAM

**Uso:** Connection pooling para reduzir overhead de conexões ao TimescaleDB

#### 3. Redis Master (telegram-redis-master)

- **Porta:** 6379 (interna), não exposta externamente
...
- **Porta:** 26379 (interna)
- **Função:** HA monitoring e failover automático
...
```bash
# Health check (via exposição direta MTProto)
...
- **Exposição:** Traefik → `http://localhost:9080/api/telegram-gateway`
- **Porta Interna:** 4010
- **Protocolo:** HTTP REST
...
- **Exposição:** Interna (sem porta publicada)
- **Retenção:** 30 dias
- **Scrape Interval:** 15s
...
```bash
# Abrir shell interativo e usar port-forward temporário
...
- **Exposição:** Interna (sem porta publicada). Planeje publicar via Traefik quando necessário.
- **Porta Interna:** 3000
...
**Dica:** utilize Prometheus para consumir as métricas via rede interna.
...
# Portas (opcional)
TELEGRAM_MTPROTO_PORT=14007  # Único serviço com porta externa direta
# Demais serviços HTTP são expostos via Traefik (http://localhost:9080/*)
```
...
bash << 'EOF'
echo "🔍 Telegram Stack - Health Check"
echo ""
...
echo ""
echo "🌐 ENDPOINTS"
curl -s http://localhost:14007/health > /dev/null && echo "  ✅ MTProto: http://localhost:14007" || echo "  ❌ MTProto: DOWN"
curl -s http://localhost:9080/api/telegram-gateway/health > /dev/null && echo "  ✅ Gateway API (Traefik): http://localhost:9080/api/telegram-gateway" || echo "  ❌ Gateway API: DOWN"
echo "  ℹ️ Prometheus, Grafana e exporters estão disponíveis apenas na rede interna (use 'docker compose port' ou Traefik quando configurado)"
EOF
```
...
4. Navegue para **Data Sources** → Verificar conexões
...
```bash
# Testar conectividade (exemplo: Gateway API → MTProto)
...
- **[Telegram Issues Summary](https://github.com/marceloterra1983/TradingSystem/blob/main/docs/TELEGRAM-ISSUES-SUMMARY.md)** - Problemas conhecidos e soluções
- **[Telegram Channels Names Issue](https://github.com/marceloterra1983/TradingSystem/blob/main/docs/TELEGRAM-CHANNELS-NAMES-ISSUE.md)** - Problema de nomes genéricos
- **[Telegram Sync Button Timeout](https://github.com/marceloterra1983/TradingSystem/blob/main/docs/TELEGRAM-SYNC-BUTTON-TIMEOUT.md)** - Timeout do botão de sync
- **[Telegram Monitoring Integration](https://github.com/marceloterra1983/TradingSystem/blob/main/docs/TELEGRAM-MONITORING-INTEGRATION.md)** - Detalhes da integração de monitoramento
...
A stack usa **minimal port exposure** para segurança:

- ✅ **Apenas portas essenciais expostas** (MTProto direto: 14007; Gateway API via Traefik em 9080)
- ✅ **Portas internas não expostas** (TimescaleDB, Redis, PgBouncer, Prometheus, Grafana, exporters)
- ✅ **Comunicação via Docker network** (telegram_backend)
...
3. **Habilite TLS/SSL** (future)
   ...
   - Gateway API: HTTPS com certificado válido
...
**Última Atualização:** 2025-11-11 15:00 BRT
