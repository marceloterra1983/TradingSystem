...
slug: /tools/infrastructure/overview
...
- **`docker-compose.5-0-database-stack.yml`** - Database tooling stack (pgAdmin, Adminer, pgWeb, QuestDB)
- **`docker-compose.4-1-tp-capital-stack.yml`** - TP Capital domain (TimescaleDB + PgBouncer + API)
- **`docker-compose.4-3-workspace-stack.yml`** - Workspace domain (PostgreSQL + services)
...
- **`docker-compose.6-1-monitoring-stack.yml`** - Monitoring stack (moved to `tools/monitoring/`)
