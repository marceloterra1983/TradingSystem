...
slug: /tools/performance/bundle-optimization-guide
sidebar_position: 1
...
|--------|--------|---------|--------|
| **Total Bundle Size** | < 3000KB | ~8000KB | ⚠️ Needs Optimization |
| **Initial Load Time** | < 3s | TBD | 📊 Monitoring |
| **Time to Interactive** | < 5s | TBD | 📊 Monitoring |
| **First Contentful Paint** | < 1.5s | TBD | 📊 Monitoring |
| **Largest Contentful Paint** | < 2.5s | TBD | 📊 Monitoring |
...
3. **Charts Vendor** - Currently 273KB, target 280KB
   - ⚠️ Consider lazy loading charts only when needed
4. **Animation Vendor** - Currently 75KB, target 80KB
   - ⚠️ Load animations on demand
...
⚠  charts-vendor: 273.28KB / 280KB (97.6%)
...
```tsx
// ❌ BAD: Import heavy components eagerly
...
```tsx
// ❌ BAD: Import large JSON files statically
import agentsCatalog from './data/agents-catalog.json';

// ✅ GOOD: Load dynamically when needed
...
```bash
# ❌ BAD: moment.js (70KB gzipped)
npm install moment

# ✅ GOOD: date-fns (2-5KB per function)
npm install date-fns

# ✅ EVEN BETTER: Native Intl API (0KB)
...
// ✅ GOOD: WebP with fallback + lazy loading
<picture>
