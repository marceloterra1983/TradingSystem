...
slug: /tools/security-config/env
...
1. `config/.env.defaults` – versioned defaults (non-sensitive, ports, image names).
2. `.env.shared` – generated host/port inventory (`npm run ports:sync`), consumed by dashboards/agents; contains zero secrets.
3. `.env` – local secrets copied from `.env.example` and filled via `scripts/env/setup-env.sh`.
4. `.env.local` – optional overrides for experiments (never committed).
5. `service/.env` – per-service overrides under `apps/*` or `backend/*` (gitignored).
...
|------|---------|-----------|--------------|
| `config/.env.defaults` | Single source of defaults and non-secret values | ✅ | Whenever a service needs a new default |
| `.env.shared` | Generated map of ports/hosts for agents, docs and dashboards | ✅ | Run `npm run ports:sync`; never hand-edit |
| `.env` | Local secrets (per developer or environment) | ❌ | Initial setup, secret rotation |
| `.env.local` | Temporary local overrides | ❌ | Debug/test scenarios only |
| `service/.env` | Service-specific overrides (Docker container scope) | ❌ | When a service cannot inherit global values |
| `.env.example` | Minimal template containing only required secrets | ✅ | When a new secret is introduced |
...
- **Governança**: `governance/controls/ENVIRONMENT-VARIABLES-POLICY.md`

> `.env.example` contém apenas segredos obrigatórios; qualquer variável não listada ali deve ter seu valor padrão definido em `config/.env.defaults`.
...
| `TP_CAPITAL_DB_PASSWORD` | Database password | – | **Yes** |
...
- **Stack**: `tools/compose/docker-compose.4-1-tp-capital-stack.yml`.  
...
| `TELEGRAM_GATEWAY_DB_PASSWORD` | Database password | – | **Yes** |
...
- **Stack**: `tools/compose/docker-compose.4-2-telegram-stack.yml`.  
...
| `WORKSPACE_DATABASE_PASSWORD` | Database password | – | **Yes** |
...
- O stack compartilhado que hospedava PgAdmin/Adminer/PgWeb foi removido em 2025-11-05. Utilize tooling local (psql, DBeaver, TablePlus) ou configure instâncias específicas dentro de cada stack dedicado conforme necessário.
...
|----------|---------|---------|
| `VITE_DASHBOARD_PORT` | Dev server port | `9080` |
...
| Variable | Description | Default | Required |
|----------|-------------|---------|----------|
| `CORS_ORIGIN` | Allowed origins (comma-separated) | `http://localhost:9080,http://localhost:3400,http://localhost:3401` | No |
...
1. **Shared variables** already in `config/.env.defaults`
   ...
   CORS_ORIGIN=http://localhost:9080  # Duplicated!
   ```
...
- [Risk Limits](risk-limits.mdx).
- [Security Audit Procedures](audit.mdx).
