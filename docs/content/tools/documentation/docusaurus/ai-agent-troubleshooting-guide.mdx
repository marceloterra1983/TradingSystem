...
slug: /tools/documentation/docusaurus/ai-agent-troubleshooting-guide
description: Playbook for AI assistants to diagnose and recover Docusaurus outages automatically.
sidebar_position: 10
...
❌ "Docs page is broken"
❌ "Can't access http://localhost:3404"
❌ "NGINX error on documentation"
```
...
❌ ERR_CONNECTION_REFUSED on port 3404
❌ docker logs docs-hub shows "404 Not Found"
❌ curl http://localhost:3404/health fails
```
...
2. **Confirm diagnosis:**
   ...
   [ -f "docs/build/index.html" ] || echo "❌ Build missing"
...
```bash
bash scripts/docs/emergency-recovery.sh && echo "✅ Recovery complete" || echo "❌ Recovery failed - check logs"
```

**Expected output:**
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚨 Docusaurus Emergency Recovery
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

ℹ️  Step 1/5: Diagnosing issue...
✅ Build exists and is recent
ℹ️  Step 2/5: Skipping build (not needed)
ℹ️  Step 3/5: Stopping docs-hub container...
ℹ️  Step 4/5: Rebuilding container...
ℹ️  Step 5/5: Verifying recovery...
✅ Health check passed!
✅ Homepage responding correctly

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ Recovery completed successfully!
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🌐 Service URL: http://localhost:3404
🏥 Health check: http://localhost:3404/health
```
...
if [ -f "docs/build/index.html" ]; then
  ...
  echo "❌ Build missing - run: cd docs && npm run docs:build"
  FAIL=1
fi
...
if [ $(find docs/build/index.html -mmin +60 2>/dev/null | wc -l) -eq 0 ]; then
  ...
  echo "⚠️  Build is old (> 1 hour) - consider rebuilding"
fi
...
if curl -sf http://localhost:3404/health > /dev/null 2>&1; then
  ...
  echo "⚠️  Current service unhealthy or not running"
fi

if [ $FAIL -eq 1 ]; then
  echo ""
  echo "❌ Pre-flight checks FAILED - do not restart container"
  exit 1
fi
...
**Verification:**
- ✅ Health check: http://localhost:3404/health
- ✅ Homepage loads: http://localhost:3404/
- ✅ No errors in logs
...
- **[Docusaurus Deployment SOP](https://github.com/marceloterra1983/TradingSystem/blob/main/governance/controls/docusaurus-deployment-sop.md)** - Complete standard operating procedures
- **[Docker Compose Configuration](https://github.com/marceloterra1983/TradingSystem/blob/main/tools/compose/docker-compose.docs.yml)** - Container setup
- **[NGINX Configuration](https://github.com/marceloterra1983/TradingSystem/blob/main/tools/compose/documentation/nginx.conf)** - Web server config
- **[Emergency Recovery Script](https://github.com/marceloterra1983/TradingSystem/blob/main/scripts/docs/emergency-recovery.sh)** - Automated fix
...
## Symptoms
- ✅ HTTP 500 on all documentation routes
- ✅ NGINX redirect loop in logs
- ✅ Empty volume mount detected
...
├─ YES → Run emergency-recovery.sh
│        ├─ Success? → Document in outputs/
│        └─ Failed? → Check Docker logs, escalate
└─ NO → Routine maintenance
         └─ Content changed?
             ├─ YES → Rebuild before restart
             └─ NO → Safe to restart
```
