---
title: .NET Overview
slug: /tools/runtime/dotnet/overview
...
> ⚠️ **Status:** Os serviços .NET estão em reestruturação e o repositório não inclui mais uma solução Visual Studio. Use esta seção como referência de arquitetura ao reconstruir os projetos.

- **Layout Proposto**: `backend/services/data-capture/` e `backend/services/order-manager/`.
- **Architecture Layers**:
...
Os exemplos abaixo assumem que os projetos `.csproj` foram recriados localmente. Ajuste os caminhos conforme a sua estrutura:

```bash
# Restaurar dependências
dotnet restore backend/services/data-capture/DataCapture.csproj

# Compilar serviço específico
...
# Executar testes
dotnet test backend/services/data-capture.Tests/DataCapture.Tests.csproj
```
