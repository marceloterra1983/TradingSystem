...
<!-- AUTO-GENERATED SECTIONS by docs:auto - DO NOT EDIT MANUALLY -->
<!-- Last generated: 2025-10-25T09:43:50.388Z -->
<!-- Source: docs_legacy/context/ops/service-port-map.md (legacy - migrated into docs/content/ops/service-port-map.md) -->
...
> 📌 **docs é a fonte primária de documentação.** A tabela ainda segue o mapeamento legado até que `docs_legacy/context/ops/service-port-map.md` seja totalmente migrado; as notas abaixo destacam as diferenças (`docs` em 3205, Docusaurus legado em 3004).
...
| Documentation API | - (npm dev server) | 3400 | http://localhost:3400 | API de documentação (Express + FlexSearch) |
| Documentation Hub (docs) | - (npm dev server) | 3205 | http://localhost:3205 | Hub de documentação (Docusaurus v3 — primário) |
| Documentação (Docusaurus v2 - Legacy) | `docs-docusaurus` | 3004 | http://localhost:3004 | Hub de documentação legado (desativação em andamento) |
| API Viewer | `docs-api-viewer` | 3101 | http://localhost:3101 | Visualização de OpenAPI/AsyncAPI |
| Library API | - (npm dev server) | 3100 | http://localhost:3100 | Workspace/ideias |
| TP Capital | - (npm dev server) | 3200 | http://localhost:3200 | Ingestão de sinais Telegram |
| B3 Market Data | - (npm dev server) | 3302 | http://localhost:3302 | Dados de mercado B3 |
...
| LangGraph | `infra-langgraph` | 8111 | http://localhost:8111 | Orquestração de agentes LangGraph |
...
| TimescaleDB | `data-timescaledb` | 5433 | localhost:5433 | Banco TimescaleDB (host → container 5432) |
| TimescaleDB Exporter | `data-timescaledb-exporter` | 9187 | http://localhost:9187 | Métricas Prometheus |
| TimescaleDB pgAdmin | `data-timescaledb-pgadmin` | 5050 | http://localhost:5050 | Administração PostgreSQL |
| TimescaleDB pgWeb | `data-timescaledb-pgweb` | 8081 | http://localhost:8081 | Cliente web leve |
| Prometheus | `mon-prometheus` | 9090 | http://localhost:9090 | Console de métricas |
| Alertmanager | `mon-alertmanager` | 9093 | http://localhost:9093 | Pipeline de alertas |
| Grafana | `mon-grafana` | 3000 | http://localhost:3000 | Dashboards pré-provisionados |
...
| 3004 | Documentação (Docusaurus v2 - Legacy) | Documentation | Legacy |
...
| 3302 | B3 Market Data API | Backend API | Active |
...
- `data-*` – Databases and storage services.  
- `infra-*` – Supporting infrastructure (LangGraph, Redis).  
- `mon-*` – Monitoring components.  
- `docs-*` – Documentation stack.  
- `firecrawl-*` – Firecrawl services.  
- `individual-*` – Legacy standalone services (deprecated).
...
3. Backend APIs: Workspace, TP Capital, B3, Documentation.  
...
| TP Capital API | http://localhost:3200/health | `{"status": "ok", "questdb": "connected"}` |
| B3 API | http://localhost:3302/health | `{"status": "ok"}` |
| Documentation API | http://localhost:3400/health | `{"status": "ok"}` |
| Documentation Hub (docs) | http://localhost:3205 | Page loads |
| Docusaurus v2 (Legacy) | http://localhost:3004 | Page loads (legacy – requires manual override) |
...
curl -f http://localhost:3103 > /dev/null 2>&1 && echo "✅ Dashboard" || echo "❌ Dashboard"
curl -f http://localhost:3100/health > /dev/null 2>&1 && echo "✅ Workspace API" || echo "❌ Workspace API"
curl -f http://localhost:3200/health > /dev/null 2>&1 && echo "✅ TP Capital API" || echo "❌ TP Capital API"
curl -f http://localhost:3302/health > /dev/null 2>&1 && echo "✅ B3 API" || echo "❌ B3 API"
curl -f http://localhost:3400/health > /dev/null 2>&1 && echo "✅ Documentation API" || echo "❌ Documentation API"
curl -f http://localhost:3205 > /dev/null 2>&1 && echo "✅ Documentation Hub (docs)" || echo "❌ Documentation Hub (docs)"
curl -f http://localhost:3004 > /dev/null 2>&1 && echo "✅ Docusaurus (legacy override)" || echo "❌ Docusaurus (legacy override)"
curl -f http://localhost:9002 > /dev/null 2>&1 && echo "✅ QuestDB" || echo "❌ QuestDB"
