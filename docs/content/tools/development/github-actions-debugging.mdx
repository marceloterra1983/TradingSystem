...
slug: /tools/development/github-actions-debugging
sidebar_position: 5
...
description: "Guia completo para acessar logs, artefatos e debugar workflows do GitHub Actions localmente"
status: active
...
```bash
gh auth login
# Siga as instruções interativas para autenticar
```
...
# Baixar logs de um workflow específico
...
✓       feat: add hybrid search      CI/CD     main    push   12345678    2m 30s   5m ago
X       fix: vitest config           Tests     main    push   12345677    1m 15s   10m ago
✓       chore: update deps           Deps      main    push   12345676    3m 45s   1h ago
```
...
**Causa comum:** Diferenças de ambiente (timezone, locale, NODE_ENV)
...
# Rodar apenas um job específico
act -W .github/workflows/code-quality.yml -j test
```

**Limitações:**
- Nem todos os recursos são suportados
- Alguns secrets podem não funcionar
...
Após corrigir o erro localmente:

1. **Testar localmente:** `npm run test` + `npm run lint` + `npm run build`
2. **Commitar correção:** `git commit -m "fix: <descrição>"`
3. **Push:** `git push`
