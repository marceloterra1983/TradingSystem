---
title: 'Telegram Gateway - Botão Checar Mensagens'
slug: /tools/troubleshooting/telegram-gateway-sync
description: "Troubleshooting guide for Telegram Gateway synchronization issues"
sidebar_position: 5
...
# Troubleshooting: Botão "Checar Mensagens" Não Funciona

## 🔍 Problema

O botão "Checar Mensagens" no dashboard do Telegram Gateway não sincroniza as mensagens dos canais monitorados.

## 🎯 Causa Raiz

**API Key não configurada** - O endpoint `/api/telegram-gateway/sync-messages` requer autenticação via header `X-API-Key`, mas a variável de ambiente não estava configurada.
...
4. Mostrar instruções para reiniciar os serviços
...
⚠️ **IMPORTANTE**: Use a **mesma key** em ambos os arquivos!
...
2. Role até o card "Mensagens"
3. Clique no botão **"Checar Mensagens"**
4. Aguarde o feedback visual:
   - ✅ Verde: Mensagens sincronizadas com sucesso
   - ❌ Vermelho: Erro na sincronização

## 📋 Checklist de Diagnóstico

- [ ] Serviço rodando na porta 4010
  ```bash
...
- [ ] Keys são idênticas em ambos os arquivos
  ```bash
...
**Causa**: Frontend não está enviando o header
**Solução**: Verificar `VITE_TELEGRAM_GATEWAY_API_TOKEN` no `.env` do frontend

### Erro: "Invalid API key"

**Causa**: Keys diferentes entre backend e frontend
**Solução**: Sincronizar as keys (use o script de configuração)

### Erro: "TELEGRAM_GATEWAY_API_KEY not configured"

**Causa**: Variável não existe no `.env` raiz
**Solução**: Adicionar `TELEGRAM_GATEWAY_API_KEY` ao `.env` raiz

### Erro: "Session not found"

**Causa**: Arquivo de sessão MTProto não existe
**Solução**: Executar autenticação
```bash
...
**Solução**: Adicionar canal via dashboard ou API
```bash
...
- [Authentication Middleware](../security-config/overview.mdx)
- [Environment Variables](/tools/security-config/env)

## 🐛 Ainda com Problemas?

Se após seguir todos os passos o botão ainda não funcionar:
...
3. **Verifique a requisição no Network**:
   - DevTools → Aba "Network"
   - Clique no botão
   - Encontre a requisição `/api/telegram-gateway/sync-messages`
   - Verifique:
     - Status Code
     - Request Headers (presença de `X-API-Key`)
     - Response Body
...
4. **Reporte o issue** com:
   ...
   - Logs do frontend
   - Screenshot do erro
   - Configuração do `.env` (sem expor a key completa)
