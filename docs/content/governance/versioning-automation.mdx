...
Follow the manual steps documented in [`VERSIONING-GUIDE.md`](./versioning-guide.mdx) when automation is unavailable. The automated script implements the same 10-step process programmatically and can be executed locally for preview (`--dry-run`) or manual commits (`--auto-commit` omitted).
...
- [`VERSIONING-GUIDE.md`](./versioning-guide.mdx) – Manual versioning process.
- [`CI-CD-INTEGRATION.md`](./ci-cd-integration.mdx) – Overview of documentation workflows.
- [`VALIDATION-GUIDE.md`](./validation-guide.mdx) – Frontmatter and maintenance validation.
