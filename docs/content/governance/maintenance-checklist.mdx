...
**Dashboard Location**: `http://localhost:3400/dashboard/` (standalone), `http://localhost:9080/documentation/metrics` (React), `http://localhost:3000/d/docs-health` (Grafana)
...
- [ ] Verificar se o workflow [`dependabot-auto-merge.yml`](https://github.com/marceloterra1983/TradingSystem/blob/main/.github/workflows/dependabot-auto-merge.yml) está ativo.
- [ ] Confirmar que PRs de `version-update:semver-patch` ou advisories de segurança entram em auto merge (`enablePullRequestAutoMerge` via GraphQL).
- [ ] Para PRs que não devem ser mesclados automaticamente, adicionar o label `blocked` ou desabilitar o auto-merge diretamente no PR.
- [ ] Registrar exceções ou merges manuais em `governance/change-log.csv` indicando motivo e follow-up.
