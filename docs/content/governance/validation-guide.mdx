...
- `workflow-lint` - Global actionlint gate (`Workflow Lint / lint` job)
...
> **How to add**: Settings → Branches → `main` protection rule → *Edit* → under **Status checks that are required**, enable “Workflow Lint / lint” (and leave the existing docs checks). Repeat for `develop` once the rule is duplicated.
