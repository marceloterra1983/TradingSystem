...
sequenceDiagram
    ...
    Orch->>Market: analyze_market(symbols, include_tp=true, include_b3=true)
    activate Market
    
    par Parallel Data Gathering
        Market->>B3: get_b3_data("PETR4")
        B3-->>Market: {price, volume, indicators}
    and
...
sequenceDiagram
    ...
    Market->>B3: get_b3_data("PETR4")
    B3-->>Market: {last_price: 32.50, volume: 1000000, ...}
...
- Applied to: All HTTP requests (B3, TP Capital, Workspace, Risk Engine)
