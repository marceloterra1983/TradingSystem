...
1. **MarketAnalysisAgent** - Analyzes B3 market data, TP Capital signals, and Workspace ideas to generate BUY/SELL/HOLD recommendations
...
**Integration**: Consumes Workspace API (port 3100), TP Capital API (port 3200), B3 API (port 3302)
...
**Internal Tools**:
- `analyze_b3_data`: Fetch and analyze B3 market data for symbol
...
- `include_b3`: Boolean flag to include B3 market data
...
- `source`: Signal origin (AGNO, TP_CAPITAL, B3)
...
- B3 price alert → Trigger analysis for affected symbol
...
### B3 Market Data API (Port 3302)

**Client**: `B3Client` (`infrastructure/adapters/b3_client.py`)

**Methods**:
- `ping()`: Health check
- `get_b3_data(symbol)`: Fetch market data snapshot for symbol
- `get_adjustments()`: Get corporate actions (splits, dividends)
- `get_gamma_levels()`: Get options gamma levels
- `get_indicators_daily()`: Get daily technical indicators

**Integration**:
- MarketAnalysisAgent fetches B3 data for requested symbols
- Filters snapshots by symbol
- Aggregates auxiliary indicators (volatility, volume, etc.)

**Resilience**: Retry with exponential backoff, circuit breaker

---

### B3 WebSocket (Port 3302/ws)

**Consumer**: `B3WebSocketConsumer` (`infrastructure/adapters/b3_websocket_consumer.py`)

**Purpose**: Receive real-time market data updates

**Callback**: `handle_b3_message` (configured callback for incoming messages)
...
**Status**: Optional (controlled by `AGNO_ENABLE_B3_WEBSOCKET`)
