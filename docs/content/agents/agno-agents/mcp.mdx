...
- `analyze_market`: Generate trading signals for symbols
  - Input: `{ "symbols": ["PETR4"], "include_tp_capital": true, "include_b3": true }`
...
- `get_b3_data`: Fetch B3 market data for symbol
  - Input: `{ "symbol": "PETR4" }`
  - Output: Market data snapshot
  - Permission scope: `b3:read`
...
**Read Permissions**:
- `market-data:read`: Access market data (B3, TP Capital)
...
dependency_status{dependency="b3_api"}
```
...
- Checks all dependencies (B3, TP Capital, Workspace, Risk Engine)
- Updates `dependency_status` gauge
- Returns: `{ "status": "healthy", "dependencies": { "b3_api": "connected", ... } }`
