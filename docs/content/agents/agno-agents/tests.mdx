...
```python
@pytest.mark.asyncio
async def test_market_analysis_agent(mock_signals, mock_openai_client):
    symbols = [entry["symbol"] for entry in mock_signals]
    signals = await analyze_market(symbols, include_tp=True)
...
async def test_agent_error_handling(monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        await analyze_market(["PETR4"], include_tp=True)
```
