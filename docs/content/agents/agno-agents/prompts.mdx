...
```json
{
  ...
  "payload": {
    ...
    "market_data": {
      ...
      "VALE3": {"last_price": 65.80, "volume": 500000, "change_pct": -1.2}
    },
    "tp_capital_signals": [
  ...
  "signals": [
    {
      ...
      "size": 100,
      "metadata": {
        "reasoning": "TP Capital signal shows buy range 32.00-33.00 with targets at 34.00, 35.00. B3 data confirms volume spike (+150%) and positive momentum. Recommend BUY at current price 32.50."
      }
    }
  ],
  "analysis_time": 0.42,
...
**Evaluation Notes**:
- LLM successfully integrated TP Capital signal with B3 market data
...
- Data sources: B3 (optional), TP Capital (optional), Workspace (optional)

**Signal Context**:
- TP Capital limit: 100 signals (configurable in client call)
- Workspace ideas: All active ideas (no limit)
- B3 data: Latest snapshot per symbol
