...
slug: /operations/runbooks/loki-troubleshooting
description: Guia operacional para investigar falhas no pipeline de logs centralizado (Promtail → Loki → Grafana).
tags:
  - observability
  - runbook
  - loki
...
```
Docker JSON Logs → Promtail (9080) → Loki (3100) → Grafana Datasource “Loki”
```
...
3. Grafana > Datasources > Loki → botão **Save & test**

## 🔍 Diagnóstico Promtail

- Logs:  
  ```bash
  docker compose -f tools/monitoring/docker-compose.yml logs -f promtail
  ```
- Conferir posição do arquivo: `/var/promtail/positions.yaml`
- Certificar que `docker.sock` está montado:
  ```bash
  docker inspect mon-promtail --format '{{ json .Mounts }}'
  ```
- Query Loki temporária:
  ```bash
...
| Mensagem | Ação |
|----------|------|
| `permission denied` | Verificar permissões do host `/var/lib/docker/containers` |
...
| Mensagem | Ação |
|----------|------|
| `failed to flush user` | Checar espaço em disco do volume `loki-data` |
| `data corruption` | Limpar cache: `rm -rf /var/loki/cache/*` e reiniciar |
| `rpc error` | Garantir `common.ring` usando `inmemory` (não misturar instâncias) |
...
- Query builder:
  ...
  ```
- Exportar logs: `Explore → Download JSON`
...
- Registrar incidente na issue aberta pelo Alert Router (se aplicável).
- Atualizar este runbook em caso de nova causa raiz.
- Considerar alerta específico no Prometheus: `loki_request_duration_seconds`.
