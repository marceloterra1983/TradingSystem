...
slug: /operations/runbooks/alertmanager-escalation
description: Procedimento operacional para responder a alertas críticos em múltiplos canais (GitHub, Slack, Email).
tags:
  - observability
  - runbook
  - alertmanager
...
> **Escopo:** alertas gerados pelo stack Prometheus → Alertmanager → Alert Router com fanout para Slack e e-mail.
...
2. **Slack** recebe mensagem no canal `{{ env "ALERTMANAGER_SLACK_CHANNEL" }}`:
   - Título: `[STATUS] ALERTNAME`
   - Emoji `:rotating_light:` para `critical`, `:warning:` para `warning`.
3. **Email** enviado para `ALERTMANAGER_EMAIL_TO` com resumo e descrição formatados em HTML.

## ✅ Primeira resposta (T+5 minutos)

1. Reconhecer o alerta no Slack com reação ✅.
2. Atribuir a issue no GitHub para o on-call.
3. Registrar no comentário da issue:
...
- [ ] Validar variáveis `ALERTMANAGER_*` no `.env`.
