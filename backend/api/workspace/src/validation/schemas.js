...
export const StatusSchema = z.enum(
  ["new", "review", "in-progress", "completed", "rejected"],
  {
    errorMap: () => ({
      message:
        "Status must be one of: new, review, in-progress, completed, rejected",
    }),
  },
);
...
export const CategorySchema = z
  ...
  .regex(
    /^[a-z0-9-]+$/,
    "Category must be lowercase letters, numbers, and hyphens only",
  );
...
export const CreateItemSchema = z.object({
  ...
  tags: z
    ...
    .optional()
    .default([])
    .refine((tags) => tags.every((tag) => tag.length > 0 && tag.length <= 50), {
 ...
 */
export const FilterItemsSchema = z
  .object({
    category: CategorySchema.optional(),
    status: StatusSchema.optional(),
    priority: PrioritySchema.optional(),
