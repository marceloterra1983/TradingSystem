...
const sharedLoggerModuleId = path.resolve(
  __dirname,
...
describe("Items Router", () => {
  ...
  beforeEach(() => {
    ...
    mockDb = {
      ...
      getCategories: jest
        .fn()
        .mockResolvedValue([{ name: "features" }, { name: "bug" }]),
      getItems: jest.fn(),
      getItem: jest.fn(),
      createItem: jest.fn(),
      updateItem: jest.fn(),
  ...
  describe("GET /api/items", () => {
    ...
    it("should handle database errors gracefully", async () => {
      mockDb.getItems.mockRejectedValue(
  ...
  describe("POST /api/items", () => {
    beforeEach(() => {
      ...
      mockDb.getCategories.mockResolvedValue([{ name: "features" }]);
    });
    ...
    it("should accept item without tags", async () => {
      ...
      await request(app).post("/api/items").send(newItem).expect(201);
