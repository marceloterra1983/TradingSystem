...
class StatsService {
  ...
  async getDashboardStats() {
    try {
      const [systemStats, ideaStats, fileStats, recentActivity] =
        await Promise.all([
          this.getSystemStats(),
          this.getIdeaStats(),
          this.getFileStats(),
      ...
      return {
        overview: {
          ...
          total_storage_mb: Math.round(
            fileStats.total_size_bytes / (1024 * 1024),
          ),
        },
        systems: systemStats,
        ideas: ideaStats,
  ...
  async getFileStats() {
    try {
      ...
      const avgFileSizeMB =
        stats.total > 0 ? stats.total_size / (1024 * 1024) / stats.total : 0;
      const downloadsPerFile =
        stats.total > 0 ? stats.total_downloads / stats.total : 0;
      ...
      return {
        ...
        total_size_mb:
          Math.round((stats.total_size / (1024 * 1024)) * 100) / 100,
        storage_efficiency: {
          small_files_percentage:
            stats.total > 0
              ? Math.round((stats.size_distribution.small / stats.total) * 100)
              : 0,
          large_files_percentage:
            stats.total > 0
              ? Math.round(
  ...
  async getAnalyticsData(timeframe = "30d") {
    try {
      const [systemStats, ideaStats, fileStats] = await Promise.all([
        this.getSystemStats(),
        this.getIdeaStats(),
        this.getFileStats(),
      ]);
      ...
      return {
        ...
        charts: {
          systems: {
            by_type: Object.entries(systemStats.by_type).map(
              ([type, count]) => ({
                label: type,
                value: count,
              }),
            ),
            by_status: Object.entries(systemStats.by_status).map(
              ([status, count]) => ({
                label: status,
                value: count,
              }),
          ...
          ideas: {
            by_status: Object.entries(ideaStats.by_status).map(
              ([status, count]) => ({
                label: status,
                value: count,
              }),
            ),
            by_category: Object.entries(ideaStats.by_category).map(
              ([category, count]) => ({
                label: category,
                value: count,
              }),
            ),
            by_priority: Object.entries(ideaStats.by_priority).map(
              ([priority, count]) => ({
                label: priority,
                value: count,
              }),
          ...
          files: {
            by_type: Object.entries(fileStats.by_mime_type).map(
              ([type, count]) => ({
            ...
            size_distribution: [
              {
                label: "Small (<1MB)",
                value: fileStats.size_distribution.small,
              },
              {
                label: "Medium (1-10MB)",
                value: fileStats.size_distribution.medium,
              },
              {
                label: "Large (10-100MB)",
                value: fileStats.size_distribution.large,
              },
              {
                label: "X-Large (>100MB)",
                value: fileStats.size_distribution.xlarge,
              },
            ],
            by_visibility: [
              ...
              {
                label: "Private",
                value: fileStats.by_visibility.private.count,
              },
            ],
          },
        },
  ...
  async globalSearch(query, filters = {}) {
    try {
      ...
      if (systems) {
        try {
          const systemResults =
            await this.systemsRepository.searchSystems(query);
