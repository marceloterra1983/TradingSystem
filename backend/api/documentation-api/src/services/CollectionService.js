...
export class CollectionService {
  constructor(config = {}) {
    ...
    this.ingestionBaseUrl = (
      ...
      process.env.LLAMAINDEX_INGESTION_URL
    ).replace(/\/+$/, "");
    this.qdrantBaseUrl = (
      config.qdrantBaseUrl ||
    ...
    this.defaultCollection =
      ...
      "documentation";
    this.defaultDocsDir =
      config.defaultDocsDir ||
      process.env.LLAMAINDEX_DOCS_DIR ||
      path.join(process.cwd(), "../../docs/content");

    // Status cache configuration
    this.statusCache = new Map();
    this.statusCacheTtl = Number(
      config.statusCacheTtl || process.env.STATUS_CACHE_TTL_MS || 30000,
    );
  }
  ...
  async getHealth() {
    ...
    const buildMessage = (source) => {
      ...
      return (
        value.error ||
        (value.data && (value.data.message || value.data.detail)) ||
        "unavailable"
      );
    };
      ...
      services: {
        query: {
          ok:
            ...
            queryHealth.value.ok &&
            (queryHealth.value.data?.status || "").toLowerCase() !== "missing",
          status:
            queryHealth.status === "fulfilled" ? queryHealth.value.status : 503,
          message: buildMessage(queryHealth),
          collection:
            queryHealth.status === "fulfilled"
              ? (queryHealth.value.data?.collection ??
                queryHealth.value.data?.configuredCollection ??
                null)
              : null,
        },
        ingestion: {
          ok:
            ingestionHealth.status === "fulfilled" &&
            ingestionHealth.value.ok &&
            (ingestionHealth.value.data?.status || "").toLowerCase() !==
              "degraded",
          status:
            ingestionHealth.status === "fulfilled"
              ? ingestionHealth.value.status
              : 503,
        ...
        ollama: {
          ...
          status:
            ollamaHealth.status === "fulfilled"
              ? ollamaHealth.value.status
              : 503,
          message:
            ollamaHealth.status === "fulfilled" && ollamaHealth.value.ok
        ...
        redis: {
          ...
          status:
            redisHealth.status === "fulfilled" ? redisHealth.value.status : 503,
          message: buildMessage(redisHealth),
        },
        collections: {
          ok:
            collectionsHealth.status === "fulfilled" &&
            collectionsHealth.value.ok,
          status:
            collectionsHealth.status === "fulfilled"
              ? collectionsHealth.value.status
              : 503,
  ...
  async _checkRedisHealth() {
    try {
      ...
      const response = await this._fetchJson(
        `${this.collectionsServiceUrl}/health`,
      );
  ...
  async getCollectionStatus(collectionName = null) {
    ...
    if (jwt) {
      ...
      const gpuResp = await this._fetchJson(`${this.queryBaseUrl}/gpu/policy`, {
        headers,
      });
  ...
  async ingestDocuments(options = {}) {
    ...
    try {
      const response = await fetch(
        `${this.ingestionBaseUrl}/ingest/directory`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async cleanOrphanChunks(collectionName = null) {
    ...
    do {
      ...
      const points = Array.isArray(page.data?.result?.points)
        ? page.data.result.points
        : [];
    ...
    for (const point of allPoints) {
      ...
      if (
        /\.(md|mdx|txt|pdf)$/i.test(normalized) &&
  ...
  async listCollections() {
    try {
      const response = await this._fetchJson(
        `${this.qdrantBaseUrl}/collections`,
      );
  ...
  async getCollectionDefinition(collectionName) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new ExternalServiceError(
          "Collections service",
          new Error("Failed to fetch collection metadata"),
          {
            statusCode: response.status,
          },
        );
