...
function parseFrontmatter(raw) {
  ...
  try {
    return { meta: yaml.load(fm[1]) || {}, body: fm[2] || "" };
  } catch {
...
async function chunkWithLlamaIndex(text) {
  let li;
  try {
    li = await import("llamaindex");
  } catch (e) {
    throw new Error("llamaindex package not installed. Run: npm i llamaindex");
  }
...
async function indexDocs() {
  ...
  console.log(
    JSON.stringify({ ok: true, files, chunks, indexer: "llamaindex" }, null, 2),
  );
}
