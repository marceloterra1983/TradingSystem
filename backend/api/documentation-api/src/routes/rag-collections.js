  ...
  collections: [
    {
      ...
      dimensions: 768,
      description:
        "Main documentation collection using Nomic Embed Text model (768 dimensions)",
      source: "docs/content",
      enabled: true,
      priority: 1,
      metadata: {
    ...
    },
    {
      ...
      dimensions: 384,
      description:
        "Documentation collection using MXBAI Embed Large model (384 dimensions, optimized for speed)",
      source: "docs/content",
      enabled: true,
      priority: 2,
      metadata: {
    ...
    },
    {
      ...
      dimensions: 768,
      description:
        "Documentation collection using Google Gemma embedding model (768 dimensions)",
      source: "docs/content",
      enabled: true,
      priority: 3,
      metadata: {
  ...
  embeddingModels: [
    {
      ...
      provider: "Nomic AI",
      description:
        "General-purpose embedding model with strong performance across domains",
    },
    {
    ...
    },
    {
      ...
      provider: "Google",
      description:
        "Google's Gemma embedding model with strong contextual understanding",
    },
  ],
  aliases: {
...
router.get("/", async (req, res) => {
  try {
    ...
    const qdrantCollectionNames = new Set(
      qdrantCollections.map((c) => c.name || c),
    );
    ...
    const enrichedCollections = await Promise.all(
      COLLECTION_CONFIG.collections.map(async (col) => {
        ...
        if (existsInQdrant) {
          try {
            const countResp = await fetch(
              `${QDRANT_BASE_URL}/collections/${encodeURIComponent(col.name)}/points/count`,
              {
                method: "POST",
        ...
        const runtimeDirectory = global.collectionDirectoryMapping?.get(
...
router.get("/models", async (req, res) => {
  try {
    ...
    const embeddingModels = ollamaModels
      ...
      .map((m) => {
        const configModel = COLLECTION_CONFIG.embeddingModels.find((cm) =>
          m.name.startsWith(cm.name),
        );
        return {
          name: m.name,
          size: m.size,
...
router.get("/:collectionName/files", async (req, res) => {
  ...
  try {
    ...
    const scrollResp = await fetch(
      `${QDRANT_BASE_URL}/collections/${encodeURIComponent(collectionName)}/points/scroll`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    const summary = {
      ...
      avgChunksPerFile:
        files.length > 0 ? Math.round(points.length / files.length) : 0,
      avgFileSizeKB:
        files.length > 0
          ? Math.round(
              files.reduce((sum, f) => sum + f.sizeBytes, 0) /
...
router.post("/:collectionName/create", async (req, res) => {
  ...
  try {
    ...
    const configCollection = COLLECTION_CONFIG.collections.find(
      (c) => c.name === collectionName,
    );

    const targetModel = embeddingModel || configCollection?.embeddingModel;
    const targetDirectory =
      sourceDirectory || configCollection?.source || "docs/content";
    ...
    const createResp = await fetch(
      `${QDRANT_BASE_URL}/collections/${encodeURIComponent(collectionName)}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    global.collectionDirectoryMapping =
      global.collectionDirectoryMapping || new Map();
    global.collectionDirectoryMapping.set(
