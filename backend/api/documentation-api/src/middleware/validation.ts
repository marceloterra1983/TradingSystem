...
export const collectionSchema = {
  body: z.object({
    name: z.string().min(1).max(100).regex(/^[a-z0-9_-]+$/, 'Name must be lowercase alphanumeric with dashes or underscores'),
    description: z.string().max(500).optional(),
    embeddingModel: z.enum(['nomic-embed-text', 'mxbai-embed-large']).default('nomic-embed-text'),
 ...
 */
export const arraySchema = <T extends ZodSchema>(itemSchema: T, min: number = 0, max: number = 100) => {
