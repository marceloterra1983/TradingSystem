...
export class DistributedLock {
  ...
  async acquire(key, timeoutMs = 30000) {
    ...
    this.logger?.info?.({ key, lockId, timeoutMs }, '[Lock] Attempting to acquire with retry');
    ...
    throw new Error(`Failed to acquire lock for ${key} after ${elapsed}ms (timeout: ${timeoutMs}ms)`);
  }
  ...
  async release(key) {
    ...
    if (!this.heldLocks.has(lockId)) {
      this.logger?.debug?.({ key, lockId }, '[Lock] Lock not held, skipping release');
      return;
    }
    ...
    try {
      ...
      if (released) {
        ...
        this.logger?.warn?.({ key, lockId }, '[Lock] Failed to release (not held)');
      }
    } catch (error) {
      this.logger?.error?.({ err: error, key, lockId }, '[Lock] Error releasing lock');
      throw error;
    }
  }
