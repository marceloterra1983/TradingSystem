...
export class StartupSyncService {
  ...
  constructor(logger) {
    ...
    this.enabled = this.parseBoolean(
      process.env.TELEGRAM_GATEWAY_SYNC_ON_STARTUP,
      true,
    );
    this.delay = parseInt(
      process.env.TELEGRAM_GATEWAY_STARTUP_SYNC_DELAY || "5000",
      10,
    );
    this.limit = parseInt(
      process.env.TELEGRAM_GATEWAY_STARTUP_SYNC_LIMIT || "500",
      10,
    );
    this.concurrency = parseInt(
      process.env.TELEGRAM_GATEWAY_STARTUP_SYNC_CONCURRENCY || "3",
      10,
    );
  }
  ...
  async runSync() {
    ...
    try {
      ...
      if (error.code === "ECONNREFUSED") {
        this.logger.error(
          "[StartupSync] MTProto service not available - check if service is running on port 4007",
        );
      } else if (error.name === "AbortError" || error.code === "ETIMEDOUT") {
        this.logger.error(
          "[StartupSync] Timeout connecting to MTProto service - sync may take longer than 90s",
        );
