...
export const measureTelegramApi = async (channelId, apiFn) => {
  ...
  try {
    ...
    const duration = (Date.now() - startTime) / 1000;
    telegramApiDurationHistogram.observe(
      { channel_id: `${channelId}:error` },
      duration,
    );
    throw error;
  }
  ...
  return {
    recordSuccess: (messageCount, savedCount) => {
      ...
      syncDurationHistogram.observe(
        { channel_id: channelId, status: "success" },
        duration,
      );
      messagesSyncedCounter.inc(
        { channel_id: channelId, source: "mtproto" },
        messageCount,
      );
    ...
    recordError: () => {
      const duration = (Date.now() - startTime) / 1000;
      syncDurationHistogram.observe(
        { channel_id: channelId, status: "error" },
        duration,
      );
    },
  };
